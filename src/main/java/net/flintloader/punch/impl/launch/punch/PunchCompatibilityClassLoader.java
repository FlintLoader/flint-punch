...
class PunchCompatibilityClassLoader extends URLClassLoader implements ClassLoaderAccess {
	private final PunchClassDelegate<PunchCompatibilityClassLoader> delegate;

	PunchCompatibilityClassLoader(boolean isDevelopment, GameProvider provider) {
		super(new URL[0], PunchCompatibilityClassLoader.class.getClassLoader());
		this.delegate = new PunchClassDelegate<>(isDevelopment, this, getParent(), provider);
	}
