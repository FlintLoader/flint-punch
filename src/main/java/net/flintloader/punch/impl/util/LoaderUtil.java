...
public final class LoaderUtil {
	...
	public static void verifyNotInTargetCl(Class<?> cls) {
		if (cls.getClassLoader().getClass().getName().equals("net.flintloader.punch.impl.launch.punch.PunchClassLoader")) {
	...
	public static void verifyClasspath() {
		try {
			List<URL> resources = Collections.list(LoaderUtil.class.getClassLoader().getResources("net/flintloader/punch/api/PunchLoader.class"));

			if (resources.size() != 1) {
				// This usually happens when fabric loader has been added to the classpath more than once.
				throw new IllegalStateException("duplicate flint loader classes found on classpath: " + resources.stream().map(URL::toString).collect(Collectors.joining(", ")));
			}
