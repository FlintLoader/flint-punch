...
public final class TinyFDPatch extends GamePatch {
	...
	public void process(PunchLauncher launcher, Function<String, ClassNode> classSource, Consumer<ClassNode> classEmitter) {
		...
		if (!launcher.getMappingConfiguration().getTargetNamespace().equals("intermediary")
				&& PunchLoader.getInstance().getMappingResolver().getNamespaces().contains("intermediary")) {
			className = PunchLoader.getInstance().getMappingResolver().mapClassName("intermediary", MORE_OPTIONS_DIALOG_CLASS_NAME);
		}
